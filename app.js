...
function seed() {
  return {
    tab: "inbox", spaceId: null, itemId: null, query: "", filter: "all",
    spaces: [
      { id: "songs", name: "Night Shift album", ico: "S", desc: "Lyrics and style prompts", mem: ["English lyrics", "No cliche tears"] },
      { id: "housing", name: "Housing Asia", ico: "H", desc: "Monthly stays, listings", mem: ["Max budget all-in"] },
      { id: "brand", name: "Brand", ico: "B", desc: "Portraits, reels, likeness", mem: ["Exact likeness only"] }
    ],
    items: [
...
$("#backBtn").onclick = function () {
  if (state.itemId) state.itemId = null; else state.spaceId = null; render();
};
$("#openLimits").onclick = function () {
  state.tab = "queue"; state.itemId = null; syncTabs(); render();
};
$("#pipClose").onclick = function () { $("#pip").hidden = true; };
document.querySelectorAll(".tab").forEach(function (btn) {
  btn.onclick = function () {
    state.tab = btn.dataset.tab; state.itemId = null;
...
function itemRow(c) {
  ...
  return '<div class="row" data-item="' + c.id + '"><div class="avatar">' + (sp ? sp.ico : "*") +
    '</div><div style="min-width:0;flex:1"><h3>' + esc(c.title) + '</h3><p>' + esc(c.body) +
    '</p></div><div class="meta">' + (c.pin ? '<div class="pin">PIN</div>' : '') + '<div>' + when(c.time) + '</div></div></div>';
}
function wireList() {
  const q = $("#q");
  if (q) q.oninput = function (e) {
    state.query = e.target.value; render();
    const n = $("#q"); n.focus(); n.setSelectionRange(state.query.length, state.query.length);
  };
  screen.querySelectorAll(".chip").forEach(function (c) { c.onclick = function () { state.filter = c.dataset.f; render(); }; });
  screen.querySelectorAll("[data-item]").forEach(function (r) { r.onclick = function () { state.itemId = r.dataset.item; render(); }; });
}
function renderInbox() {
  $("#topSub").textContent = "Inbox"; $("#backBtn").hidden = true;
  const list = filtered();
  ...
  screen.innerHTML = '<div class="install">Safari Share then Add to Home Screen.</div>' +
    '<div class="search"><span>o</span><input id="q" placeholder="Search notes and captures" value="' + esc(state.query) + '" /></div>' +
    '<div class="chips">' + chips + '</div><div class="section">' + list.length + ' items</div>' +
...
function renderSpaces() {
  ...
  screen.querySelectorAll("[data-space]").forEach(function (b) { b.onclick = function () { state.spaceId = b.dataset.space; render(); }; });
...
function newSpaceSheet() {
  ...
  $("#saveSpace").onclick = function () {
    const name = $("#sn").value.trim(); if (!name) return;
    const id = uid("s");
    state.spaces.push({ id: id, name: name, ico: "#", desc: $("#sd").value.trim() || "Workspace", mem: [] });
    save(); closeSheet(); state.spaceId = id; state.tab = "spaces"; syncTabs(); render();
  };
}
function renderSpace() {
  ...
  screen.innerHTML = '<p style="color:var(--mute);font-size:13px;margin-bottom:10px">' + esc(s.desc) + '</p>' +
...
function renderItem() {
  ...
  $("#topSub").textContent = s ? s.name : "Note"; $("#backBtn").hidden = false;
  screen.innerHTML = '<input class="field" id="it" value="' + esc(c.title) + '" /><textarea id="ib">' + esc(c.body) + '</textarea>' +
    ...
    '<button class="cta" id="saveItem">Save</button><button class="ghost" id="exportThis">Export this note</button>';
  $("#saveItem").onclick = function () { c.title = $("#it").value.trim() || c.title; c.body = $("#ib").value; c.time = Date.now(); save(); toast("Saved on this phone"); };
  $("#pin").onclick = function () { c.pin = !c.pin; save(); render(); };
  $("#del").onclick = function () { state.items = state.items.filter(function (x) { return x.id !== c.id; }); state.itemId = null; save(); render(); };
  $("#exportThis").onclick = function () { download(c.title.replace(/\s+/g, "_") + ".md", "# " + c.title + "\n\n" + c.body + "\n"); };
}
function editItem(spaceId) {
  ...
  $("#sn").onclick = function () {
    const title = $("#nt").value.trim(); const body = $("#nb").value.trim(); if (!title && !body) return;
    state.items.unshift({ id: uid("i"), space: spaceId || (state.spaces[0] && state.spaces[0].id) || null, type: "note", title: title || "Untitled", body: body, pin: false, time: Date.now() });
    save(); closeSheet(); render();
  };
}
function renderCapture() {
  $("#topSub").textContent = "Capture"; $("#backBtn").hidden = true;
  screen.innerHTML = '<div class="section">Dump into a space</div><div class="cap-grid">' +
    ...
    '<button class="cap" data-cap="share"><div class="big">P</div><h3>Paste in</h3><p>Link or text</p></button></div>';
  screen.querySelectorAll("[data-cap]").forEach(function (b) { b.onclick = function () { runCap(b.dataset.cap); }; });
}
function runCap(kind) {
  ...
  if (kind === "task") {
    ...
    $("#sq").onclick = function () {
      const title = $("#qt").value.trim(); if (!title) return;
      state.queue.unshift({ id: uid("q"), title: title, status: "queued" }); save(); closeSheet(); toast("Parked in queue");
    };
    return;
  }
  if (kind === "voice") { state.tab = "voice"; syncTabs(); render(); return; }
  ...
  $("#sp").onclick = function () {
    const body = $("#ps").value.trim(); if (!body) return;
    state.items.unshift({ id: uid("i"), space: space, type: "capture", title: body.slice(0, 48), body: body, pin: false, time: Date.now() });
    save(); closeSheet(); toast("Captured"); state.tab = "inbox"; syncTabs(); render();
  };
}
function renderQueue() {
  $("#topSub").textContent = "Queue"; $("#backBtn").hidden = true;
  ...
  state.queue.forEach(function (q) {
    rows += '<div class="q-item"><div class="dot' + (q.status === "done" ? " ok" : "") + '"></div><div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:600">' +
      ...
      (q.status !== "done" ? '<button class="mini" data-done="' + q.id + '">Done</button><button class="mini" data-x="' + q.id + '">Cancel</button>' : '') + '</div></div>';
  });
  screen.innerHTML = '<div class="stat"><div class="k">Open jobs</div><div class="v">' + n + '</div><div class="reset">Nothing here is deleted when you leave.</div></div>' +
    (rows || '<div class="empty">Queue empty.</div>') + '<button class="cta" id="addQ">Add job</button>';
  screen.querySelectorAll("[data-done]").forEach(function (b) {
    b.onclick = function () { const q = state.queue.find(function (x) { return x.id === b.dataset.done; }); if (q) q.status = "done"; save(); render(); };
  });
  screen.querySelectorAll("[data-x]").forEach(function (b) {
    b.onclick = function () { state.queue = state.queue.filter(function (x) { return x.id !== b.dataset.x; }); save(); render(); };
  });
  $("#addQ").onclick = function () { runCap("task"); };
}
function renderVoice() {
  $("#topSub").textContent = "Voice"; $("#backBtn").hidden = true;
  screen.innerHTML = '<div class="voice-stage"><div class="cam"><div class="live-tag">NOTE</div><div class="wave"><i></i><i></i><i></i><i></i><i></i></div></div>' +
  ...
  $("#saveV").onclick = function () {
    const body = $("#vt").value.trim(); if (!body) return;
    state.items.unshift({ id: uid("i"), space: (state.spaces[0] && state.spaces[0].id) || null, type: "voice", title: body.slice(0, 48), body: body, pin: false, time: Date.now() });
    save(); toast("Saved"); state.tab = "inbox"; syncTabs(); render();
  };
  $("#pipBtn").onclick = function () { $("#pip").hidden = false; $("#pipLine").textContent = "Draft open"; state.tab = "inbox"; syncTabs(); render(); };
}
function exportMd(list, name) {
  download(name, ["# KLEYN export", ""].concat(list.map(function (c) { return "## " + c.title + "\n" + c.body + "\n"; })).join("\n"));
}
